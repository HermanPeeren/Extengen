 ...
 */
    // A concept (for instance: a Product or a User) is an object.
    // An object has an id and name and an objectType (for instance: Entity or Page).

    // Variable names in this file use the word "object".
    // Some methods in this file, like updateConceptReferencesList() and backupConceptID() use the word "concept".

    // An objectMap maps the id of an object to a JS-object with its name and an index.
    // The objectMaps are made in updateConceptReferencesList() for every objectType.
    // Per objectType we also have a list of options for a dropdown-list, giving all possible references.

    // The objectTypeMap maps objectTypes to objectMaps. For every objectType there is an objectMap
    ...
    // The referencer can be the same as the referenced object (in which case they can be left out of the objectReferencerMap).
    // todo: Maybe still list them for completenes of everything that can be referenced?
    // Those objectReferencerMaps are per language apart in a JS-file, generated by the projectforms-generator.
    // todo: when combining multiple languages, then include/combine those objectReferencerMaps

    // Options are all possibilities for a reference, per objectType (= first index).
    // todo: this should probably also be integrated in the objectTypeMap
    let options = [];
     ...
     * The list is put in an objectMap for that objectType, which is put in the objectTypeMap.
     *
     * Also update the options referring to all instances of that objectType in a dropdown-list.
     * Then it calls setRefOptions() to put the options in the DOM.
     */
    function updateConceptReferencesList(objectType) {
        ...
        // Initialise the (empty) options-list for that objectType
        ...
        objectNameList.forEach(instanceName => {
            ...
            let object_id = document.getElementById(instanceName.id.replace('_name', '_id'));
            ...
            // make the options with <object>_ids as values and <object>_names visible in the list
            options[objectType] += `<option value=${object_id.value}>${instanceName.value}</option>` + '\n';
    ...
    function setChildRefOptions(childName, parentName) {
        ...
        objectTypeMap.forEach(function(objectMap, objectType) {

            // Start a string for the options of this objectType
            options[objectType] = '<option value="0">&nbsp;</option>';

            objectMap.forEach(function(object,objectId) {

                objectName = object.name;

                // <object>_name has a hidden var <object>_type
                object_type = document.getElementById(objectId.replace(objectType.toLowerCase() + '_name', objectType.toLowerCase() + '_type'));
                ...
                // Add the option with object_id as value and object_name visible in the list
                options[objectType] += `<option value=${objectId}>${objectName}</option>` + '\n';
    ...
    function backupConceptID(event) {
        ...
        if (event.target.value == 999999) {
            ...
            updateConceptReferencesList(objectType);
            return;
        }
        ...
        // todo: get the childName and parentName (not only for Entity and Field)
        const childName = 'Field';
        const parentName = 'Entity';
    ...
    window.onload = function () {
        ...
        // Init list of objects to refer to
   ...
   */
            // Init all references to this object. For instance: references to entities and pages in ER1.
            updateConceptReferencesList(objectType[0]);
    ...
    /**
     * Update list with name of objects for an objectType when name of an object of that objectType changes.
     * Make the options for that objectType.
     * todo: kan dit weg??? dan options maken bij updateConceptReferencesList()
     */
    function editObjectList(objectType) {

        objectNames = [].slice.call(document.querySelectorAll('.' + objectType.toLowerCase() + 'Name'));

        // make the options with <object>_ids as values and <object>_names visible in the list
        options[objectType] = '<option value="0">&nbsp;</option>';

        // get the <object>_id of every <object>, creating an uuid if <object>_id doesn't exist
        objectNames.forEach(object => {

            objectName = object.name;

            // <object>_name has a hidden var <object>_id
            objectId = document.getElementById(objectName.id.replace(objectName +'_name', objectName +'_id'));

            // give that <object>>_id an uuid if empty // todo....
            if (!objectId.value) {
                objectId.value = uuidv4();
            }

            //objectIds.push(entity_id); // todo /...

            // get the entity_id of every entity, creating an uuid if entity_id doesn't exist
            options[objectType] += `<option value=${entity_ids[i].value}>${entities[i].value}</option>` + '\n';
        })
