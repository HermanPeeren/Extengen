...
(function () {
  ...
  /**
    ...
    * View element calls jSelectExtengen when a extengen is clicked
    * jSelectExtengen creates the link tag, sends it to the editor,
    * and closes the select frame.
    */

  window.jSelectExtengen = function (id, title, catid, object, link, lang) {
    var hreflang = '';

    if (!Joomla.getOptions('xtd-extengen')) {
      // Something went wrong
      window.parent.Joomla.Modal.getCurrent().close();
      return false;
    }

    var _Joomla$getOptions = Joomla.getOptions('xtd-extengen'),
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    for (var i = 0, l = elements.length; l > i; i += 1) {
      ...
      elements[i].addEventListener('click', function (event) {
        ...
        if (functionName === 'jSelectExtengen') {
          // Used in xtd_extengen
